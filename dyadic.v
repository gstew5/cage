...
Proof.
  unfold Qred.
  generalize (Z.ggcd_gcd x (Zpos (pow_pos 2 n))).
  generalize (Z.ggcd_correct_divisors x (Zpos (pow_pos 2 n))).
  destruct (Z.ggcd x (Zpos (pow_pos 2 n))) as [a [b c]]; simpl.
  intros [H0 H] H2 H3.
  subst a.
  assert (H2: Z.gcd x (Zpos (pow_pos 2 n)) = 1%Z).
